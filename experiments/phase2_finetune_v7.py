"""
Phase 2 v7: v4 base + tail-weighted loss + more epochs.

v4 is the proven best (VaR 1% hourly: 0.6%).
v6 showed skipping 5min hurts (VaR 1% went back to 10.6%).
v7 = v4 + v6's tail weighting + more epochs. Keep 5min stage.

Changes from v4:
  - Tail-weighted pinball loss (P10/P90 3x, P20/P80 2x)
  - More epochs: 25/25/40 (vs 15/15/30)
  Everything else identical to v4.

Usage:
  python experiments/phase2_finetune_v7.py --stage progressive
"""
import argparse
import json
import logging
...
# Same as v4 but more epochs
STAGE_CONFIG = {
    "5min":   {"epochs": 25, "lr": 5e-5, "batch_size": 64, "stride": 64},
    "hourly": {"epochs": 25, "lr": 3e-5, "batch_size": 64, "stride": 24},
...
def tail_weighted_pinball_loss(pred, target):
    """Pinball loss with 3x weight on P10/P90, 2x on P20/P80."""
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Phase 2 v7: v4 + tail-weighted loss + more epochs")
    logger.info("  5min → hourly → daily (keep 5min!)")
    logger.info("  Freeze 15, crypto 50%%, tail weight P10/P90 3x")
    logger.info("=" * 60)
    ...
    for stage in stages:
        ...
        if stage != stages[0]:
            ...
            prev_ckpt = MODELS / f"timesfm_v7_{prev}_best.pt"
            if prev_ckpt.exists():
        ...
        model.train()
        ckpt = MODELS / f"timesfm_v7_{stage}_best.pt"
    ...
    if all(results.get(s, {}).get("status") == "OK" for s in stages):
        final = MODELS / "timesfm_v7_progressive_best.pt"
        torch.save(model.state_dict(), final)
        logger.info("Final: %s", final)

    out = {
        "phase": "2_v7",
        "recipe": "v4_base + tail_weighted_pinball + more_epochs",
        "changes_from_v4": [
            "Tail-weighted pinball: P10/P90 3x, P20/P80 2x",
            "More epochs: 25/25/40 (was 15/15/30)",
        ],
        "kept_from_v4": [
            "5min → hourly → daily progressive (5min is critical!)",
            "Freeze 15/20",
            "Last-patch-only loss",
            "50% crypto / 50% tradfi",
            "No augmentation",
        ],
        "stages": results,
    }
    (RESULTS / "phase2_finetune_v7.json").write_text(json.dumps(out, indent=2, default=str))
