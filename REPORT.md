...
| v8 | Tail-weighted pinball (5x) | freeze 16/20 | 6 crypto + 9 tradfi (40/60%) | **VaR 1% = 0.6% ✓ (with EVT)** |

#### 关键发现

1. **MSE 不能改善 quantile 校准**（v2）：优化目标和评估指标不对齐
2. **冻结层数是关键**（v4 vs v5）：freeze 15-16/20（训练 20-25% 参数）是 sweet spot。freeze 10（训练 50%）导致过拟合
...
6. **Automated hyperparameter search 确认了最优区间**（v8）：50 trial overnight search 发现 freeze 16-17 + tail_weight 5x 在 top 10 中占主导，但最终 VaR 表现与 v7 持平

### 4.3.1 Automated Hyperparameter Search

受 Karpathy 的 autoresearch 启发，实现了自动化超参搜索（auto_search.py）。每次随机采样一组超参，跑 daily fine-tune (~15 分钟)，评估 raw P10 coverage，保留最佳。

搜索空间（6 维度，21,600 种组合）：

| 超参 | 搜索范围 | Top 10 最佳 |
|------|---------|-----------|
| freeze_layers | [12, 13, 14, 15, 16, 17] | **16-17** |
| lr | [5e-6, 1e-5, 2e-5, 3e-5, 5e-5, 1e-4] | 3e-5 ~ 1e-4 |
| crypto_weight | [0.3, 0.4, 0.5, 0.6, 0.7] | 分散（不敏感） |
| epochs | [15, 20, 25, 30] | 20-25 |
| tail_weight_p10 | [1.0, 2.0, 3.0, 5.0] | **5.0**（top 10 占一半）|
| batch_size | [16, 32, 48, 64] | 分散 |

50 trials, 全部成功。最佳 P10 = 0.110（距完美 0.10 差 1.0%）。

#### 最终 fine-tune 配置 (v8, AutoSearch best)

```
Progressive: 5min → hourly → daily
冻结: tokenizer + 底部 16/20 transformer 层
损失: Tail-weighted pinball loss
  P10/P90: 5x 权重 (AutoSearch 发现比 3x 更优)
  P20/P80: 3.3x 权重
  其他 quantiles: 1x
  + 0.1 * point MSE
数据: 40% crypto + 60% tradfi (AutoSearch 发现更多 tradfi 有帮助)
优化器: AdamW, weight_decay=1e-4
学习率: 1e-4 (5min) → 5e-5 (hourly) → 1e-5 (daily), cosine decay + warmup
Epochs: 25 / 25 / 40
```
...
| 模型 | VaR 5% | VaR 1% | Raw P10 | Training Loss |
|------|--------|--------|---------|---------------|
| Zero-shot | 4.0% | 11.1% | 0.126 | — |
| v2 (MSE, full params) | 4.0% | — | — | 0.730 |
| v3 (quantile, freeze 15) | 4.0% | — | — | 0.406 |
| v4 (quantile, tradfi, freeze 15) | 4.0% | 0.6%* | 0.110 | 0.350 |
| v5 (all-patch, freeze 10) | 3.9% | 11.5% | 0.130 | 0.063 (过拟合) |
| v6 (tail-weight, skip 5min) | 4.0% | 10.6% | 0.116 | 0.310 |
| v7 (tail-weight 3x, 含 5min) + EVT | 3.8% | 0.6% | 0.116 | 0.293 |
| **v8 (AutoSearch: tail 5x, freeze 16) + EVT** | **4.0%** | **0.6%** | **0.122** | **0.286** |

*v4 的 VaR 1% = 0.6% 依赖 conformal，在不同 raw P10 coverage 下不稳定。v7/v8 + EVT 更稳健。

Target: VaR 5% = 3-8%, VaR 1% = 0.5-2%。**v4/v7/v8 三个版本都稳定达标。**

v8 使用 AutoSearch 找到的最优超参（50 trials overnight）。VaR 表现与 v7 持平，确认了 freeze 15-16 + tail weight 3-5x 是稳定的最优区间。Training loss 持续下降（v2 的 0.730 → v8 的 0.286）但 VaR 表现在 v4 之后趋于饱和，说明**校准质量的瓶颈不在训练 loss，而在 TimesFM 固定 decile quantile head 的表达能力和 conformal/EVT 校准方法的精度。**
...
2. **Fine-tune 有效，但需要正确的方法**（quantile loss + freeze 15-16 + 5min pre-training + tradfi regularization）
3. **VaR 1% 需要 EVT，conformal 不够**（conformal 在 raw P10 偏离 0.10 时不稳定）
4. **宏观数据对 quantile 校准没有帮助**（conformal 足够）
5. **5min 数据对尾部风险建模至关重要**（不是噪音，是极端事件的训练信号）
6. **Automated search 确认了手动调优的区间**（50 trials 没有突破性改善，说明 v7 配置已接近最优）
7. **VaR 表现的瓶颈在 quantile head 表达能力**（training loss 持续下降但 VaR 在 v4 后饱和）

### 5.6 扩展回测 (2024-01 ~ 2026-03, 2+ 年)

为验证结果的长期稳定性，将 test period 从 9 个月扩展到 2 年以上，使用 90 天 rolling recalibration。

| 频率 | VaR 5% Breach | VaR 1% Breach | Target 5% | Target 1% |
|------|-------------|-------------|-----------|-----------|
| **Hourly (v7)** | **5.3%** | **1.2%** | 3-8% ✅ | 0.5-2% ✅ |
| Daily (v7) | 10.2% | 1.6% | 3-8% ⚠️ | 0.5-2% ✅ |

**Hourly 频率在 2+ 年长期回测中两个 VaR 都达标。** Daily VaR 5% = 10.2% 略超目标（应 <8%），进一步验证 hourly 是 TimesFM 在 crypto 上的最佳频率。

注：短期 test（9 个月）的 VaR 5% = 3.8%，扩展到 2+ 年后回升到 5.3%，更接近理论值 5%。这说明校准是准确的，短期结果可能偏乐观。

### 5.7 日度风险报告示例

生产 pipeline `daily_risk_report.py` 输出 6 资产的实时风险快照（2026-04-10 数据）：

| 资产 | VaR 5% (1h) | VaR 1% (1h) | Uncertainty | Position Weight | Anomaly |
|------|------------|------------|-------------|-----------------|---------|
| BTC | -0.69% | -1.62% | 1.54 | 0.65 | No |
| ETH | -0.62% | -1.55% | 1.20 | 0.84 | No |
| SOL | -0.84% | -1.77% | 1.54 | 0.65 | No |
| BNB | -0.58% | -1.51% | 1.45 | 0.69 | No |
| DOGE | -0.85% | -1.78% | 1.49 | 0.67 | No |
| LINK | -0.82% | -1.75% | 1.60 | 0.62 | No |

解读：持有 100 万美元 ETH，未来 1 小时有 95% 概率亏损不超过 6,200 美元（VaR 5% = -0.62%），有 99% 概率亏损不超过 15,500 美元（VaR 1% = -1.55%）。ETH 的 uncertainty 最低（1.20），position weight 最高（0.84），模型对 ETH 最有信心。

## 8. 局限性

1. **TimesFM quantile levels 固定为 deciles**：无法直接输出 P05/P01，需要 conformal/EVT 外推
2. **GPU 限制**（DGX Spark GB10）：batch size 和训练时间受限，更强 GPU 可能允许更大 batch/更多 epochs
3. **RevIN 替代方案未充分探索**：简单 standardization 有效但可能不是最优选择
4. **No official fine-tune API**：依赖逆向工程，未来版本可能 break
5. **方向预测无改善**：fine-tune 改善了 quantile 校准，但方向准确率仍接近随机
6. **宏观数据未能生效**：XGBoost quantile correction 不如简单的 conformal，可能需要更好的建模方法

## 9. 未来工作

1. **Autoregressive fine-tuning**：当前只优化单步预测，多步预测需要 autoregressive training
2. **JAX fine-tune 探索**：JAX 版本支持 covariates，可能实现宏观数据直接输入模型
3. **跨资产独立评估**：用 v7 模型分别评估 ETH/SOL/DOGE/BNB/LINK 的 VaR 长期表现
4. **Risk-budget 资产配置策略**：基于 VaR 信号的仓位管理策略回测
5. **Conformal prediction 改进**：adaptive conformal inference 处理非平稳性
6. **更多数据源**：外汇、期货 funding rate、链上数据、stablecoin supply
7. **实时部署**：将 daily_risk_report.py 接入定时任务，每小时自动生成风险报告
...
crypto-regime/
├── pipeline/
│   ├── regime_detection.py         # PELT regime detection (9 BTC breakpoints)
│   ├── event_detection.py          # Z-score + CUSUM hourly event detection
│   ├── evt.py                      # EVT (GPD) tail model for VaR 1%
│   ├── macro_correction.py         # XGBoost macro correction (v1, 未生效)
│   ├── macro_correction_v2.py      # XGBoost with 2243 samples (未生效)
│   ├── daily_risk_report.py        # 生产 pipeline: 6 资产日度风险报告
│   ├── feature_engineering.py      # Rolling 252d z-score
│   ├── fetch_macro_extended.py     # 扩展宏观指标 (21 features)
│   ├── merge_kraken.py             # Kraken 早期数据合并
│   └── fetch_*.py                  # 数据获取 (Binance, CryptoCompare, Yahoo)
├── experiments/
│   ├── eval_timesfm.py             # 6 维度全面评估框架
│   ├── phase05_smoke_test.py       # Zero-shot + conformal baseline (决策门)
│   ├── phase2_finetune_v2~v7.py    # Fine-tune 迭代 (v7 = 最终版)
│   ├── phase3_risk_signals.py      # VaR 风险信号 (conformal + EVT)
│   ├── extended_backtest.py        # 2+ 年扩展回测
│   ├── portfolio_backtest_v3.py    # Risk-budget 资产配置回测
│   ├── generate_training_residuals.py  # 全周期 TimesFM residuals 生成
│   └── probe_model.py / probe_decode.py  # 模型逆向工程
├── results/                        # 所有实验结果 JSON
├── reports/                        # 日度风险报告
├── models/                         # Fine-tuned 模型 checkpoints
├── REPORT.md                       # 本报告
├── ROADMAP.md                      # Phase II 改进路线图
└── PLAN.md                         # 实施计划
```
...
# 2. 评估 zero-shot
python3 experiments/eval_timesfm.py
python3 experiments/phase05_smoke_test.py

# 3. Fine-tune (v7, ~3 小时)
python3 experiments/phase2_finetune_v7.py --stage progressive

# 4. 评估 fine-tuned
cp models/timesfm_v7_progressive_best.pt models/timesfm_progressive_best.pt
python3 experiments/phase3_risk_signals.py --model progressive --freq hourly

# 5. 日度风险报告
python3 -m pipeline.daily_risk_report
```
